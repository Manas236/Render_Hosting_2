...
git_pusher_bp = Blueprint('git_pusher_bp', __name__)
...
<script>
  ...
  async function pushImage() {
    ...
    try {
      const res  = await fetch('{{ url_for("git_pusher_bp.push") }}', { method: 'POST', body: formData });
...
@git_pusher_bp.route('/push', methods=['POST'])
def push():
...
if __name__ == '__main__':
    ...
    test_app.register_blueprint(git_pusher_bp)
