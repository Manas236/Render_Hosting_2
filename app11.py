...
def _find_cards(soup):
    ...
    intro_sum = soup.find(
        "td", style=lambda s: s and "font-size:13px" in s and "padding:0" in s)
    intro_img = soup.find("img", width="754")
    if intro_hl and intro_sum and intro_img:
        cards.append(MockCard(0, intro_hl, intro_sum,
                     intro_img, category_text="Featured"))

    # 2. Event Cards
    cat_tds = soup.find_all(
        ["td", "div"], style=lambda s: s and "letter-spacing:1.5px" in s and "text-transform:uppercase" in s)
