...
def sanitize_hex_color(raw: str):
    ...
    Returns the cleaned value on success, or None if the value is not a
    valid hex color. The caller must skip the style update when None is returned
    so that an invalid form value can never corrupt an existing inline style.
    """
    v = raw.strip()
    if re.match(r"^#([0-9a-fA-F]{3}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})$", v):
        return v
    logging.warning("sanitize_hex_color: unexpected value %r — skipping update", v)
    return None
...
def _estimate_card_height(card_td, container_width_px: float) -> float:
    ...
    # --- Content row: find <td class="card-content"> by class ----------
    content_td = card_table.find("td", class_="card-content")
    if not content_td:
        return total_h

    # Padding of the content td (padding-bottom is 0 before equalization)
    ...
    total_h += pad_top + pad_bot  # pad_bot == 0 before equalization

    # Available content width (subtract horizontal padding and 2px for borders)
    if len(pad_parts) >= 2:
    ...
    # --- Measure p/h2 blocks inside div.card-inner -------------------
    # Read More is in a separate row and is excluded here (same height for all cards)
...
def equalize_card_heights(soup) -> None:
    ...
    for tr in all_trs:
        ...
        for td, h in zip(card_tds, heights):
            ...
            # Find the inner card table, then locate the content td by class
            card_table = td.find("table", recursive=True)
            if not card_table:
                continue

            # Find <td class="card-content"> directly — no heuristic needed
            content_td = card_table.find("td", class_="card-content")
            if not content_td:
                continue

            # Apply padding-bottom to the content <td>.
            # Strips any previously injected value to avoid compounding on re-runs.
            ...
            logging.info("Card equalization: added %dpx padding-bottom to card-content td", pad_px)
...
<div id="main">
  ...
  <div id="panel">
    <form id="eform" method="POST" action="/editor/save">
      ...
      <!-- Hidden field: serialized preview DOM with JS equalization applied, written by submitForm('download') -->
...
function submitForm(action) {
  ...
  if (action === 'download') {
    // Serialize the live preview iframe DOM — which already has JS equalization
    // applied via equalizeCardHeights() — so the download is pixel-identical
    // to the preview. The backend will strip editor attrs and scripts.
    const iframe = document.querySelector('iframe');
    const domField = document.getElementById('dom_html');
    if (iframe && iframe.contentDocument && domField) {
      domField.value = iframe.contentDocument.documentElement.outerHTML;
    }
  }
...
def _process_form(form_data) -> str:
    ...
    for sc in style_controls:
        ...
        if ctype == "color" or ctype == "accent-color":
            ...
            if new_val is None:
                continue  # invalid hex — preserve the existing style unchanged
        elif ctype == "number":
            new_val = new_val + "px"
        elif ctype == "url":
...
(function() {
  ...
  //
  // MEASUREMENT: Use .card-inner (a plain <div>) — NOT <td> or <table>.
  // Table cells in the same <tr> stretch to match the tallest cell, so
  // any td/table measurement returns the row height, not the content height.
  // A div is unaffected by table-layout and reports true content height.
  //
  // STRUCTURE: Each card's inner table has three rows:
  //   1. Image row   (fixed height)
  //   2. Content row (<td class="card-content"> → <div class="card-inner">)
  //   3. Read More   (separate row, anchored at bottom, NOT measured)
  //
  // PADDING: Applied only to <td class="card-content"> (the content row td).
  // This pushes the content cell taller while keeping Read More anchored.
  ...
  function equalizeCardHeights() {
    ...
    for (var t = 0; t < allTrs.length; t++) {
      ...
      // Collect direct <td class="col-stack"> children of this row
      var cardTds = [];
      ...
      // Gather references to the content td and its card-inner div per card
      var cards = [];
      for (var i = 0; i < cardTds.length; i++) {
        var ctd   = cardTds[i].querySelector('.card-content'),
            inner = ctd ? ctd.querySelector('.card-inner') : null;
        cards.push({td: cardTds[i], ctd: ctd, inner: inner});
      }

      // 1. Reset any previously injected padding-bottom so measurement is clean
      for (var i = 0; i < cards.length; i++) {
        if (cards[i].ctd) cards[i].ctd.style.paddingBottom = '';
      }

      // 2. Force reflow, then measure .card-inner height.
      // .card-inner is a <div> — immune to table-row equalization stretching —
      // so its height equals the true content height (category + headline + summary).
      void tr.offsetHeight;
      var heights = [];
      for (var i = 0; i < cards.length; i++) {
        heights.push(cards[i].inner
          ? cards[i].inner.getBoundingClientRect().height
          : 0);
      }
      var maxH = Math.max.apply(null, heights);

      // Validation log: if textLength differs but height is the same, inner is null
      // and we are still measuring a stretched container — check the HTML structure.
      var dbg = [];
      for (var i = 0; i < cards.length; i++) {
        dbg.push({card: i,
                  height: Math.round(heights[i]),
                  textLength: cards[i].inner ? cards[i].inner.innerText.length : -1,
                  measured: cards[i].inner ? '.card-inner' : 'MISSING'});
      }
      console.log('Card row (' + cards.length + '): max=' + Math.round(maxH), dbg);

      // 3. Apply padding-bottom to the content <td> of shorter cards only.
      // This makes all content cells the same height; the Read More row below
      // stays at its natural fixed height, correctly anchored at the bottom.
      for (var i = 0; i < cards.length; i++) {
        var diff = maxH - heights[i];
        if (diff < 2 || !cards[i].ctd) continue;
        var nb = Math.round(diff);
        cards[i].ctd.style.paddingBottom = nb + 'px';
        console.log('  Card ' + i + ': +' + nb + 'px -> card-content padBot=' + nb + 'px');
      }
    }
  }
...
def preview():
    ...
    # Card height equalization is performed exclusively by the JS
    # equalizeCardHeights() after the real DOM is rendered (getBoundingClientRect).
    # No backend estimation — JS is the single source of truth.
...
@require_login
def download():
    """Stream the newsletter HTML as a download.

    When the request carries a 'dom_html' field the frontend has serialized the
    live preview iframe (already equalized by JS).  The backend strips editor
    attributes and the injected editor script, then returns the result.

    Falls back to _process_form() (form-field reconstruction) when no DOM is
    provided (e.g. programmatic calls).
    """
    ...
    if dom_html:
        ...
        # Remove every <script> element — they were injected by the editor and
        # must not appear in the final newsletter (the original template has none).
        for script in soup.find_all("script"):
            script.decompose()

        # Strip all editor-only data attributes stamped during preview rendering
        strip_editor_attrs(soup)

        html = format_html(soup)
    else:
        # Fallback: reconstruct from form fields (no JS equalization)
