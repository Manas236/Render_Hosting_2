...
def load_elements(file_path: str = None):
    ...
    for node in soup.descendants:
        ...
        tag = node.name
        if tag not in config.ALL_EDITABLE:
            continue
        ...
        if tag == "img":
            ...
            if _has_editable_descendant(node):
                continue
            if tag == "td":
                non_br_kids = [c for c in node.children if getattr(
                    c, "name", None) and c.name != "br"]
                if non_br_kids:
                    continue
...
def load_style_controls(soup) -> list:
    ...
    def _find_hex(style_str: str, prop: str):
        m = re.search(
            rf"(?<![a-zA-Z-]){re.escape(prop)}\s*:\s*(#[0-9a-fA-F]{{3,8}})", style_str)
...
def equalize_card_heights(soup) -> None:
    ...
    for tr in all_trs:
        ...
        heights = [_estimate_card_height(td, cw)
        ...
        for td, h in zip(card_tds, heights):
            ...
            existing_style = re.sub(
                r';\s*padding-bottom\s*:[^;]*', '', existing_style, flags=re.I)
            existing_style = re.sub(
                r'^padding-bottom\s*:[^;]*;?\s*', '', existing_style, flags=re.I)
            content_td["style"] = existing_style.rstrip(
...
def _estimate_card_height(card_td, container_width_px: float) -> float:
    ...
    pad_left = pad_parts[3] if len(pad_parts) == 4 else (
        pad_parts[1] if len(pad_parts) >= 2 else (pad_parts[0] if pad_parts else 0.0))
    pad_right = pad_parts[1] if len(pad_parts) >= 2 else (
        pad_parts[0] if pad_parts else 0.0)
    ...
    for block in content_root.find_all(["p", "h2"]):
        ...
        lh_px = (_px(lh_raw) if _px(lh_raw) >= 4 else fs *
                 _px(lh_raw)) if lh_raw else fs * 1.4
        avg_char_w = fs * 0.52
        ...
        mt = mb = (m_parts[0] if len(m_parts) == 1 else (
            m_parts[0] if len(m_parts) == 2 else m_parts[0])) if m_parts else 0.0
        if len(m_parts) >= 3:
            mb = m_parts[2]
...
def stamp_logo_spacer(soup) -> None:
    ...
    for td in sidebar.find_all("td"):
        ...
        try:
            ...
            continue
        if [c for c in td.children if getattr(c, "name", None)]:
            continue
...
def _process_form(form_data, file_path: str = None) -> str:
    ...
    for elem in elems:
        ...
        if etype == "img":
            old_src = el.get("src", "")
            base = config.GITHUB_BASE if old_src.startswith(
                config.GITHUB_BASE) else ""
            full = base + new_val if base else new_val
            if full.strip() != old_src.strip():
                el["src"] = full
        elif etype == "bg-img":
            old_url = elem["text"]
            base = config.GITHUB_BASE if old_url.startswith(
                config.GITHUB_BASE) else ""
            full = base + new_val if base else new_val
            if full.strip() == old_url.strip():
                continue
            ...
            if new_val.strip() != el.get("href", "").strip():
                el["href"] = new_val
        else:
            new_html = superscript_ordinal(
                fix_date_br(new_val.replace("\n", "<br>")))
            if re.sub(r"\s+", "", _normalize_br(new_html)) == re.sub(r"\s+", "", _normalize_br(elem.get("inner_html", ""))):
                continue
            el.clear()
    ...
    for sc in style_controls:
        ...
        if key not in form_data:
            continue
        new_val, el, prop, ctype = form_data[key].strip(
        ), sc["el"], sc["prop"], sc["type"]
        if not new_val:
            continue
            ...
            if spacer_height >= 0:
                ...
                if spacer_td:
                    ...
                    spacer_td["style"] = update_style_prop(
                        spacer_td.get("style", ""), "height", f"{spacer_height}px")
        except ValueError:
            pass
...
def _render_style_control(sc: dict) -> str:
    sid, label, ctype, prop, val, sc_id = sc["style_id"], sc[
        "label"], sc["type"], sc["prop"], sc["value"], sc["id"]
    if ctype in ("color", "accent-color"):
        hex6 = val.lower() if re.match(
            r"^#[0-9a-fA-F]{6}$", val) else "#000000"
        palette = ["#c8102e", "#c8781a", "#d4920f", "#8d3222", "#6d4c41", "#bf360c", "#1c4f7c", "#1565c0", "#37474f",
                   "#00796b", "#2a7a5c", "#1b6e3a", "#8b2a6e", "#4a148c", "#6a1c4b", "#3d5a80", "#2d5986", "#1a1a1a"]
        swatches = "".join(
            f'<button type="button" class="swatch{"  swatch-active" if c == hex6 else ""}" style="background:{c}" data-color="{c}" data-sid="{sid}" data-prop="{prop}" data-ctype="{ctype}" title="{c}"></button>' for c in palette)
